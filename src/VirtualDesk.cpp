...
const VDeskLayout& VirtualDesk::activeLayout(const RememberLayoutConf& conf, const CSharedPointer<CMonitor>& exclude) {
    if (!activeIsValid) {
        activeIsValid = true;
...
VDeskLayout& VirtualDesk::searchActiveLayout(const RememberLayoutConf& conf, const CSharedPointer<CMonitor>& exclude) {
...
CSharedPointer<CMonitor> VirtualDesk::deleteInvalidMonitor(const CSharedPointer<CMonitor>& monitor) {
    VDeskLayout layout_copy(layouts[m_activeLayout_idx]);
...
void VirtualDesk::checkAndAdaptLayout(VDeskLayout* layout, const CSharedPointer<CMonitor>& exclude) {
    auto enabledMons = currentlyEnabledMonitors(exclude);
    if (enabledMons.empty())
        return;
    for (const auto& [mon, wid] : VDeskLayout(*layout)) {
...
VDeskLayout VirtualDesk::generateCurrentMonitorLayout() {
    VDeskLayout layout;
